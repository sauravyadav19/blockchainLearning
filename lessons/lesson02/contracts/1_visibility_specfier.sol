...
contract ParentHouse{
    ...
    duration of the transactions, it is stored in memory, as this complex and requires allocation and deallocation
    ...
    but when we are marking a resouce(function) with public visibility specifer we are being ambiogous in the sense of where 
    this function can be called from,compiler assumes the complex and the worst case which is that it might be called from
    within the contract and hence it is required to keep that temporary data location into more "permanent" data location
    and hence copies it to memory and as copying of the data requires space with is expensive and hence increasing the gas fee
    and making it costly, this may seems like a small thing at first but if you imagine that function called multiple times 
